...
function renderCase(item: IgdAdjudicationDataset['cases'][number], index: number, total: number, questions: string[]): string {
  ...
  return `<article class="case" data-case-id="${esc(item.id)}" data-index="${index}" hidden>
    <header class="case-head">
      ...
      <span class="status-pill">BELUM DIADJUDIKASI</span>
    </header>
    ...
    <section class="review">
      <h3>Checklist keputusan dokter</h3>
...
<title>M13-14 IGD · Adjudikasi PRIMERA</title>
<style>
...
@media(max-width:760px){.toolbar{grid-template-columns:1fr}.toolbar-right{justify-content:flex-start}.case-head{display:block}.status-pill{display:inline-block;margin-top:.5rem}.choices{grid-template-columns:1fr}.decision-options{grid-template-columns:repeat(2,1fr)}.choice-head{display:block}.choice-head span{display:block;margin-top:.3rem}.case{padding:.85rem}}
...
<body>
<header class="masthead"><div class="masthead-inner"><h1>M13-14 IGD · Paket Adjudikasi Dokter</h1><p>Empat belas prototipe IGD mode Karier. Snapshot <code>${esc(data.sourceCommit)}</code> · release <code>${esc(data.contentRelease)}</code> · engine ${data.engineRevision} · fingerprint <code>${data.artifactFingerprint.slice(0, 16)}…</code></p><div class="warning-banner"><strong>Bukan bukti bahwa kasus sudah benar.</strong> Kompilator hanya memeriksa struktur dan provenance. Aktivasi akademik tetap menunggu keputusan dokter pada keenam pertanyaan review.</div></div></header>
<div class="toolbar-wrap"><div class="toolbar"><div class="toolbar-left"><select id="case-select" aria-label="Pilih kasus">${data.cases.map((item, index) => `<option value="${esc(item.id)}">${index + 1}. ${esc(item.name)}</option>`).join('')}</select><span class="progress" id="progress" aria-live="polite"></span></div><div class="toolbar-right"><button id="previous" type="button">← Sebelumnya</button><button id="next-undecided" class="primary" type="button">Belum diputuskan berikutnya</button><button id="export" type="button">Ekspor JSON</button><label><span class="primary" style="display:inline-block;padding:.65rem .9rem;border-radius:6px;cursor:pointer">Impor JSON</span><input id="import" type="file" accept="application/json,.json" hidden></label><button id="theme" type="button">Tema gelap</button></div></div></div>
