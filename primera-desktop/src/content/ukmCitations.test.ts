...
describe('M11 UKM C2 - cakupan sitasi kunjungan rumah', () => {
  ...
  it('memetakan seluruh keluarga kartu kegiatan tanpa bergantung pada engine beku', () => {
    // Sapuan delivery 2026-08-02: urutan komponen dlm prosa boleh berubah
    // (kini "25 keterampilan" mendahului tahun kurikulum) — yang dijaga test
    // ini kelengkapan sitasi, bukan urutan kalimat.
    const posy = sumberKegiatanUkm({ id: 'posy_timbang' }, 'posyandu')!
    expect(posy).toMatch(/Kementerian Kesehatan RI/i)
    expect(posy).toContain('2023')
    expect(posy).toContain('2024')
    expect(posy).toMatch(/25 keterampilan/i)
  ...
  it('menjaga lapisan ILP terhubung ke bukti implementasi SAJI terkini', () => {
    ...
    // Sapuan delivery 2026-08-02: jargon Inggris diterjemahkan sesuai kebijakan
    // "bahasa pemain" — test mengikuti padanannya, bukan mempertahankan Inggris.
    expect(panduan).toMatch(/layanannya terlewat.*tidak patuh.*tanda bahaya/is)
  })
})
