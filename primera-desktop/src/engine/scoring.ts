...
/**
 * Grade huruf debrief malam. SENGAJA TETAP DI BERKAS BEKU.
 *
 * Pemisahan 2026-08-22: perakitan KALIMAT debrief pindah ke `debriefNarator.ts`
 * yang TIDAK dibekukan, supaya memperbaiki tata bahasa/typo pada catatan malam
 * tak lagi memaksa bump REVISI_ENGINE — dulu satu koma pun menggeser hash
 * scoring.ts dan menjatuhkan seluruh dosier berjalan ke
 * "tidak_dapat_diverifikasi". Yang TIDAK ikut pindah adalah fungsi ini:
 * ambang 3,5/2,5/1,5 adalah ATURAN PENILAIAN, bukan narasi. Kalau ia ikut
 * keluar, ambang grade harian jadi bisa digeser diam-diam di berkas tak-beku.
 * Batasnya persis di situ: angka tinggal di sini, kata-kata pindah ke sana.
 */
export function gradeHarian(state: GameState): string {
  const hasilResmi = state.klinik.selesaiHariIni.filter((item) => !item.formativePrototype)
  if (hasilResmi.length === 0) return '—'
  const rata = hasilResmi.reduce((jml, p) => jml + NILAI_GRADE[p.grade], 0) / hasilResmi.length
  return rata >= 3.5 ? 'A' : rata >= 2.5 ? 'B' : rata >= 1.5 ? 'C' : 'D'
}
