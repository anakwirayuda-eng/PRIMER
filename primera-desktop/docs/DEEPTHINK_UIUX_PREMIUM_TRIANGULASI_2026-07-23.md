...
| Q2 | Tunda paket penuh pasca-playtest | Konsisten dgn rekomendasi awal; tanpa klaim kode. | **DIPUTUSKAN dr. Wirayuda 2026-07-23: TUNDA pasca-playtest M13-1b.** Fullscreen F11 + ingat ukuran/posisi jendela TIDAK dikerjakan pra-kelas; dibuka kembali hanya bila playtest menemukan kebutuhan nyata. (Min-size 1200×760 sudah terpasang — lihat Q1.) |
| Q3 | SFX diegetik "kertas/stempel", bukan synth | Arah desain valid, TAPI: implementasi kini synth murni (`synth.ts`); foley = butuh aset audio berlisensi jelas (disiplin `check:bgm-license`) + effort aset. Keputusan arah produk. | **DIPUTUSKAN dr. Wirayuda 2026-07-23: setuju DT.** Arah terkunci (kosakata diegetik: gesek kertas / stempel basah / coretan pulpen / sunyi utk Karma-Kode Hitam). Implementasi = paket kerja terpisah ber-prasyarat aset berlisensi; BELUM dikerjakan. |
| Q4 | Default SFX mati/rendah utk kelas | Default kini `volumeSfx: 0.8` (`settings.ts:27`) — usulan DT = perubahan default nyata. Keputusan produk. | **DIPUTUSKAN dr. Wirayuda 2026-07-23: setuju DT — DIEKSEKUSI.** `volumeSfx` default 0.8 → **0.2** (sangat rendah; naikkan = opt-in). Pengguna lama tak terdampak (localStorage menang). Bila mau MATI total (0), satu baris. |
...
| Q6 | MejaKerja dulu, lalu Kunjungan | Konvergen; penjadwalan tetap hak product owner. | **DIPUTUSKAN dr. Wirayuda 2026-07-23: MejaKerja dulu — DIEKSEKUSI** (audit baris-per-baris, temuan di bagian bawah dokumen/commit terkait). Kunjungan menyusul. |
