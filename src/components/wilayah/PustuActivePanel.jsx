...
// ═══════════════════════════════════════════════════════════════
const ANC_CHECKS = [
    { id: 'berat_badan', label: 'Timbang Berat Badan', icon: 'BB', time: 1 },
    { id: 'tekanan_darah', label: 'Ukur Tekanan Darah', icon: 'TD', time: 1 },
    { id: 'tinggi_fundus', label: 'Ukur Tinggi Fundus Uteri', icon: 'TFU', time: 1 },
    { id: 'denyut_jantung_janin', label: 'Dengar DJJ (Doppler)', icon: 'DJJ', time: 2 },
    { id: 'hb', label: 'Cek Hemoglobin', icon: 'HB', time: 2 },
    { id: 'golongan_darah', label: 'Cek Golongan Darah', icon: 'ABO', time: 2 },
    { id: 'protein_urin', label: 'Tes Protein Urin', icon: 'PU', time: 2 },
    { id: 'gds', label: 'Gula Darah Sewaktu', icon: 'GDS', time: 2 },
    { id: 'hiv', label: 'Rapid Test HIV', icon: 'HIV', time: 3 },
    { id: 'hbsag', label: 'Tes HBsAg', icon: 'HBs', time: 2 },
    { id: 'sifilis', label: 'Tes Sifilis (RPR)', icon: 'RPR', time: 2 },
    { id: 'letak_janin', label: 'Palpasi Leopold (Letak Janin)', icon: 'LP', time: 2 },
    { id: 'rencana_persalinan', label: 'Rencana Persalinan (P4K)', icon: 'P4K', time: 1 },
];
...
// 🏥 MAIN COMPONENT
// ═══════════════════════════════════════════════════════════════
export default function PustuActivePanel({ onClose, onComplete, buildingType = 'pustu' }) {
    ...
    const tx = (key, options = {}) => t(`wilayahContent.ui.pustuActive.${key}`, options);
    const getVisitLabel = (key) => tx(`visitLabels.${key}`, { defaultValue: ANC_VISITS[key]?.label || key });
    const getCheckLabel = (key) => tx(`ancChecks.${key}`, { defaultValue: ANC_CHECKS.find(c => c.id === key)?.label || key });
    const getRiskLabel = (key) => tx(`riskFactors.${key}`, { defaultValue: RISK_FACTORS[key]?.label || key });
    const getEventLabel = (event) => tx(`events.${event?.id}.label`, { defaultValue: event?.label || '-' });
    const getEventDescription = (event) => tx(`events.${event?.id}.description`, { defaultValue: event?.description || '' });
    const getKbName = (key, method) => tx(`kbMethods.${key}.name`, { defaultValue: method?.name || key });
    const getKbDuration = (key, method) => tx(`kbMethods.${key}.duration`, { defaultValue: method?.duration || '-' });
    const getKbSideEffect = (key) => tx(`kbSideEffects.${key}`, { defaultValue: String(key).replaceAll('_', ' ') });
    const getKbContraindication = (key) => tx(`kbContraindications.${key}`, { defaultValue: String(key).replaceAll('_', ' ') });
    const formatCheckList = (checks = []) => checks.map(getCheckLabel).join(', ');
    const formatVisitFeedback = (result, currentVisitType = visitType) => {
        if (!result) return '';
        const missed = result.checksMissed || [];
        if (missed.length === 0) return tx('feedback.ancComplete');
        const essential = ANC_VISITS[currentVisitType]?.essentialChecks || [];
        if (missed.length <= Math.max(1, Math.floor(essential.length / 3))) {
            return tx('feedback.ancMissing', { count: missed.length, checks: formatCheckList(missed) });
        }
        return tx('feedback.ancManyMissing', { checks: formatCheckList(essential) });
    };
    const getLogFeedback = (log) => (
        log.feedbackKey
            ? tx(`feedback.${log.feedbackKey}`, { defaultValue: log.feedback || '' })
            : (log.visitResult ? formatVisitFeedback(log.visitResult, log.visitType) : (log.feedback || ''))
    );
    const getLogHandlerLabel = (log) => tx(`handlers.${log.handlerKey || (log.handler === 'Village Midwife' ? 'midwife' : 'doctor')}`, { defaultValue: log.handler || '' });
    ...
    const handleDelegateKader = (patient) => {
        ...
        setSessionLog(prev => [...prev, {
            patient,
            handlerKey: 'midwife',
            handler: 'Village Midwife',
            score: isError ? 15 : 30,
            feedbackKey: isError ? 'midwifeError' : 'midwifeOk',
            feedback: isError
                ? 'The village midwife missed an important examination. Pregnancy risk was not detected.'
                : 'The village midwife completed a standard examination. Notes are OK.'
        }]);
    ...
    const finalizeVisit = useCallback((riskRes, kbRes = null) => {
        ...
        setSessionLog(prev => [...prev, {
            ...
            feedbackKey: totalScore >= 70 ? 'doctorStrong' : totalScore >= 40 ? 'doctorPartial' : 'doctorPoor',
            feedback: totalScore >= 70 ? 'ANC examination is complete and aligned with standards.' :
                totalScore >= 40 ? 'Some important examinations were missed.' :
                    'The examination is very incomplete. Pregnancy risk was not identified.'
        }]);
                        ...
                        <div className="flex items-center gap-4 relative z-10">
                            ...
                            <div>
                                <div className="font-mono text-[9px] text-pink-400 tracking-[0.3em] uppercase mb-0.5 flex items-center gap-2 font-bold">
                                    <span className="w-1.5 h-1.5 rounded-full bg-pink-500" /> {tx('header.badge')}
                                </div>
                                        ...
                                        <div key={patient.id} className="bg-slate-900 border border-slate-700 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-lg animate-in slide-in-from-bottom-4 group hover:border-pink-800 transition-colors" style={{ animationDelay: `${idx * 100}ms` }}>
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-slate-400 text-[10px] font-mono uppercase tracking-widest mt-1 flex gap-3">
                                                        <span>{tx('patient.age', { age: patient.age })}</span>
                                                        <span>G{patient.ancData?.gravida}P{patient.ancData?.parity}A{patient.ancData?.abortus}</span>
                                                        <span>{tx('patient.gestationalWeekShort', { week: patient.ancData?.gestationalWeek })}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="flex gap-2 mt-4 sm:mt-0 w-full sm:w-auto">
                                                <button onClick={() => handleDelegateKader(patient)} className="flex-1 sm:flex-none px-4 py-3 rounded-lg bg-slate-800 border border-slate-600 text-slate-400 font-bold text-[10px] uppercase tracking-widest hover:text-white hover:border-slate-500 flex items-center justify-center gap-2 btn-med border-b-slate-900">
                                                    <Baby size={14} /> {tx('actions.delegateMidwife')}
                                                </button>
                        ...
                        <div className="w-full h-full flex animate-in slide-in-from-right-8 bg-[#1e232d]">
                            ...
                            <div className="flex-[3] p-8 overflow-y-auto">
                                <div className="kia-pink rounded shadow-xl border border-pink-300 p-6 mb-6 transform rotate-[0.5deg]">
                                    <h3 className="font-serif text-pink-900 font-bold text-xl uppercase tracking-widest mb-2">{tx('ancCard.title', { visit: visitType })}</h3>
                                    <div className="grid grid-cols-2 gap-2 font-mono text-[10px] text-pink-800 uppercase tracking-widest">
                                        <div>{tx('ancCard.name')}: <b>{activePatient.name}</b></div>
                                        <div>{tx('ancCard.age')}: <b>{tx('patient.ageYears', { age: activePatient.age })}</b></div>
                                        <div>G{activePatient.ancData?.gravida}P{activePatient.ancData?.parity}A{activePatient.ancData?.abortus}</div>
                                        <div>{tx('ancCard.gestationalWeek')}: <b>{tx('patient.weekValue', { week: activePatient.ancData?.gestationalWeek })}</b></div>
                                        <div>{tx('ancCard.edd')}: <b>{activePatient.ancData?.edd}</b></div>
                                        <div>{tx('ancCard.startWeight')}: <b>{activePatient.ancData?.weightStart} kg</b></div>
                                        <div>{tx('ancCard.height')}: <b>{activePatient.ancData?.height} cm</b></div>
                                ...
                                <div className="bg-slate-900 border border-slate-700 rounded-xl p-5">
                                    <h4 className="text-white font-black text-sm uppercase tracking-widest flex items-center gap-2 mb-4">
                                        <ClipboardList size={16} className="text-pink-400" /> {tx('ancChecksTitle')}
                                    </h4>
                                    <p className="text-slate-400 text-[9px] font-mono uppercase tracking-widest mb-4">
                                        {tx('ancChecksHint', { visit: getVisitLabel(visitType) })}
                                    </p>
                                        ...
                                        {ANC_CHECKS.map(check => {
                                            ...
                                            return (
                                                <button key={check.id} onClick={() => toggleCheck(check.id)}
                                                    ...
                                                    <span className="text-[9px] font-black text-pink-300 bg-slate-950/60 px-1.5 py-0.5 rounded">{check.icon}</span>
                                                    <span className="flex-1 uppercase tracking-widest font-bold">{getCheckLabel(check.id)}</span>
                                                    {isEssential && <span className="text-[8px] bg-pink-900 text-pink-300 px-1.5 py-0.5 rounded uppercase font-bold">{tx('requiredVisit', { visit: visitType })}</span>}
                                                </button>
                                            );
                                        })}
                                    </div>
                            ...
                            <div className="w-72 border-l border-slate-700 p-6 flex flex-col bg-slate-900/80 z-20 shadow-[-20px_0_50px_rgba(0,0,0,0.5)]">
                                <div className="mb-6">
                                    <h3 className="text-white font-black text-sm uppercase tracking-widest flex items-center gap-2">
                                        <Stethoscope size={16} className="text-pink-400" /> {getVisitLabel(visitType)}
                                    </h3>
                                    <p className="text-slate-400 text-[9px] font-mono mt-2 uppercase tracking-widest leading-relaxed">
                                        {tx('submitPanel.required', { count: essentialChecks.length })}<br />
                                        {tx('submitPanel.selected', { count: selectedChecks.length })}
                                    </p>
                                </div>

                                <div className="flex-1 flex flex-col justify-between">
                                    <div className="bg-black/40 p-3 rounded-lg border border-slate-800 mb-4">
                                        <div className="font-mono text-[9px] text-slate-500 uppercase tracking-widest mb-2">{tx('submitPanel.completeness')}</div>
                                        ...
                                        <div className="font-mono text-[9px] text-right mt-1 text-slate-400">{tx('submitPanel.requiredCount', { done: selectedChecks.filter(c => essentialChecks.includes(c)).length, total: essentialChecks.length })}</div>
                                    </div>
                            ...
                            <div className="max-w-3xl w-full">
                                <div className="border-l-4 border-amber-500 pl-4 mb-6">
                                    <h3 className="text-white font-black text-xl uppercase tracking-widest flex items-center gap-2">
                                        <ShieldAlert size={20} className="text-amber-500" /> {tx('risk.title')}
                                    </h3>
                                ...
                                {visitResult && (
                                    <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 mb-6">
                                        <div className="font-mono text-[9px] text-pink-400 uppercase tracking-widest mb-2 font-bold">{tx('risk.visitResult', { visit: visitType })}</div>
                                        <div className="text-slate-300 text-xs font-mono">{formatVisitFeedback(visitResult)}</div>
                                            ...
                                            <div className="mt-3 space-y-2">
                                                {visitResult.events.map((evt, i) => (
                                                    <div key={i} className="bg-amber-950/40 border border-amber-500/30 rounded p-2 text-amber-300 text-[10px] font-mono">
                                                        {tx('warningPrefix')} {getEventLabel(evt)}: {getEventDescription(evt)}
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                                ...
                                <div className="space-y-2 mb-6">
                                    {Object.entries(RISK_FACTORS).map(([key, factor]) => (
                                        <button key={key} onClick={() => toggleRisk(key)}
                                            ...
                                            <span className="flex-1 uppercase tracking-widest font-bold">{getRiskLabel(key)}</span>
                                            <span className="text-[8px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded">{tx('risk.weight', { weight: factor.weight })}</span>
                                        </button>
                                    ))}
                                </div>
                        ...
                        <div className="w-full h-full p-8 overflow-y-auto bg-[#1e232d] animate-in slide-in-from-right-8 flex flex-col items-center">
                            <div className="max-w-3xl w-full">
                                <div className="border-l-4 border-emerald-500 pl-4 mb-6">
                                    <h3 className="text-white font-black text-xl uppercase tracking-widest flex items-center gap-2">
                                        <Heart size={20} className="text-emerald-400" /> {tx('kb.title')}
                                    </h3>
                                    <p className="text-slate-400 font-mono text-[10px] tracking-widest uppercase mt-1">
                                        {tx('kb.hint')}
                                    </p>
                                    ...
                                    {Object.entries(KB_METHODS).map(([key, method]) => (
                                        <button key={key} onClick={() => setSelectedKB(key)}
                                            ...
                                            <div className="flex justify-between items-center mb-1">
                                                <span className="text-white font-bold text-sm uppercase tracking-widest">{getKbName(key, method)}</span>
                                                <span className="font-mono text-[10px] bg-emerald-900 text-emerald-400 px-2 py-0.5 rounded">{tx('kb.effectiveness', { value: method.effectiveness })}</span>
                                            </div>
                                            <div className="font-mono text-[9px] text-slate-400 uppercase tracking-widest">
                                                {tx('kb.duration', { duration: getKbDuration(key, method) })} | {tx('kb.sideEffects', { effects: method.sideEffects.length > 0 ? method.sideEffects.map(getKbSideEffect).join(', ') : tx('kb.minimal') })}
                                            </div>
                                            {method.contraindications.length > 0 && (
                                                <div className="font-mono text-[9px] text-red-400 uppercase tracking-widest mt-1">
                                                    {tx('warningPrefix')} {tx('kb.contraindications', { items: method.contraindications.map(getKbContraindication).join(', ') })}
                                                </div>
                                            )}
                                        </button>
                                    ))}
                                    ...
                                    {sessionLog.map((log, i) => (
                                        <div key={i} className={`bg-slate-50 p-4 border-l-4 shadow-sm rounded flex flex-col gap-1.5 ${log.score < 40 ? 'border-red-600' : log.score < 70 ? 'border-amber-500' : 'border-emerald-500'}`}>
                                            <div className="flex justify-between font-bold text-slate-900 uppercase">
                                                <span>{log.patient.name} <span className="text-[9px] bg-slate-200 px-1.5 py-0.5 rounded ml-1">{getLogHandlerLabel(log)}</span></span>
                                                <span className={log.score >= 70 ? 'text-emerald-600' : log.score >= 40 ? 'text-amber-600' : 'text-red-600'}>
                                                    {tx('report.score', { score: log.score })}
                                                </span>
                                            </div>
                                            <div className="text-[10px] text-slate-600">{getLogFeedback(log)}</div>
                                            {log.visitType && <div className="text-[9px] text-pink-600 font-bold">{tx('report.visitSummary', { visit: getVisitLabel(log.visitType), count: (log.checksPerformed || []).length })}</div>}
                                            {log.events?.length > 0 && log.events.map((evt, j) => (
                                                <div key={j} className="text-[9px] text-amber-700 bg-amber-50 p-1 rounded">{tx('warningPrefix')} {getEventLabel(evt)}</div>
                                            ))}
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="mt-8 pt-6 border-t-2 border-pink-800/30 flex justify-between items-center">
                                    <div className="flex gap-6">
                                        ...
                                        <div className="text-center">
                                            ...
                                            <div className="font-mono text-[9px] uppercase tracking-widest text-slate-500 font-bold">{tx('report.reputation')}</div>
                                        </div>
                                    </div>
                                    <button onClick={() => {
