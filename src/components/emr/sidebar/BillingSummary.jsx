    ...
    return (
        <div className="space-y-3 px-3 pb-1 border-t border-current/5">
            <span className={`text-[10px] font-black uppercase tracking-widest ${isDark ? 'text-emerald-400' : 'text-slate-500'}`}>
                {t('emrWorkspace.billingSummary.title')}
            </span>
            <div className={`p-4 rounded-2xl border-2 transition-all duration-500 ${isBPJS
                ...
                <div className="flex justify-between items-end mb-2">
                    <div>
                        <p className={`text-[10px] font-black uppercase tracking-widest mb-0.5 ${isBPJS ? 'text-cyan-600' : 'text-orange-600'}`}>{t('emrWorkspace.billingSummary.totalService')}</p>
                    ...
                    <div className="text-right">
                        <p className={`text-[10px] font-black uppercase tracking-widest mb-0.5 ${isBPJS ? 'text-cyan-600' : 'text-orange-600'}`}>{t('emrWorkspace.billingSummary.patientPay')}</p>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-current/10">
                    ...
                    <p className={`text-[9px] font-black uppercase tracking-wider ${isBPJS ? 'text-cyan-600' : 'text-orange-600'}`}>
                        {isBPJS ? t('emrWorkspace.billingSummary.bpjsCoverage', { class: social.bpjsClass }) : t('emrWorkspace.billingSummary.generalCoverage')}
                    </p>
                </div>
            </div>
        </div>
