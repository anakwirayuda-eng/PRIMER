        ...
        <div className={`rounded-2xl border overflow-hidden animate-slideRight ${isDark ? 'bg-slate-900 border-slate-800/80 shadow-lg shadow-black/20' : 'bg-white border-slate-200 shadow-sm'}`}>
            <div className={`p-3 border-b flex justify-between items-center ${isDark ? 'bg-slate-800/20 border-slate-800' : 'bg-slate-50 border-slate-100'}`}>
                <div className="overflow-hidden">
                    ...
                    <p className="text-[9px] text-slate-500 mt-0.5 font-bold">{localize(m.category)}</p>
                </div>
                <button
            ...
            <div className="p-3 space-y-3">
                <div className="flex items-center justify-between">
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{t('emrWorkspace.treatment.dosageRule')}</span>
                    <div className={`px-2 py-0.5 rounded-lg text-xs font-black ${isDark ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-50 text-emerald-700'}`}>
                        {t('emrWorkspace.treatment.timesDaily', { count: m.frequency })}
                    </div>
                </div>
                ...
                <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{t('emrWorkspace.treatment.duration')}</span>
                        <span className={`text-[10px] font-black ${isDark ? 'text-amber-400' : 'text-amber-700'}`}>{t('emrWorkspace.treatment.days', { count: m.duration })}</span>
                    </div>
                    <input
                        type="range"
                        min="1"
            ...
            <div className={`px-3 py-2 flex justify-between items-center ${isDark ? 'bg-slate-800/40 text-slate-400' : 'bg-slate-50 text-slate-500'}`}>
                <span className="text-[9px] font-black uppercase tracking-widest">{t('emrWorkspace.treatment.totalDispense')}</span>
                <span className={`text-xs font-black ${isDark ? 'text-white' : 'text-slate-800'}`}>
                    {m.frequency * m.duration} <span className="text-[10px] opacity-50 font-medium lowercase">{t('emrWorkspace.treatment.tablets')}</span>
                </span>
            </div>
        </div>
    );
